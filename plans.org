...
*** errors
  - there are many kinds of errors you can make with python.
    - syntax errors: before the program runs, checking the code is
      typed in correctly and grammatically makes sense.
    - traceback errors: during the program's execution, it just cannot
      anymore. Something went wrong.
    - close reading the error message.
      4
*** partners practice: make all the errors, 15 min
...
- 2 ways of getting htmml elements
  - dot syntax: soup.title, .p. .h1, .a
  - find() method
- getting just the text
  - .text
  - looping through elements with find_all()

*** BREAK

*** digging into HTML elements
- the HTML inspector
  - inspecting elements
...
*** narrowing down elements by text, class, href
  - To access the attributes like ~class~, we use the syntax:
    ~tag['attr']~)For example, we can search for CSS classes using
    this syntax.

#+begin_src python
  soup.h3['class'] #['chakra-heading', 'css-1vygpf9']
  soup.h3.a['href']
#+end_src

- Once we have a `class` value, then we can get more granular about
  our searching. Here, we would use the `find()` method. This is
  useful when there are a lot of objects with the same element, and
...
*** extracting data about the bills
- saving this data to a csv, for storage, analysis, etc.
- multiple steps:
  1. isolate bill card data
    - titles, captions, categories (we will add desc. and links later) 
  2. pick out the information we want from the data
  3. process information into lists
  4. save to CSV 

First, create a new object called bill_cards, which enables us to
narrow down the parts of the website that we want to scrape.

#+begin_src python
  bill_cards = soup.find_all('div', class_ ='css-4rck61')

  # use a loop to check that we have all the right data
  for i in bill_cards:
    print(i.text)
    
#+end_src

Second, pick out information from each bill card. Now that we have
narrowed down our data to bill_cards, we can search within this code
for the individual elements we want.

Using the inspector, take 5-10 minutes create a list of html elements
and attributes that correspond to the above information. Work in
partners.
...
Now, we want to save our data to a list. How might we do that?
...
*** Start to work on the challenge
** week 6: apis
Agenda
...
- viewing data
  ...
  - selecting column syntax, dot or brackets
